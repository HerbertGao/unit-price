...
**Reason**: `limit` / `offset` / `category` 三个参数随本变更移除,本需求规定的参数边界没有作用对象。

**Migration**: 分页由客户端本地切片承载、cohort 准入由客户端据 `categoryNodes` 的 `comparableUnit` 前置拦截(见 `api-client`);服务端按 `(per100ml, unit_price.id)` 下发,客户端继承该序、不重排。本需求中与分页无关的一条约束——**品类 slug 校验集必须编译期派生自 `packages/db` 的 `CATEGORY_NODES`、禁止手写第二份枚举、禁止运行期查 `tag` 表**——其唯一存续消费方是 `/compute`,已由本变更的 `compute-api` delta 承接,不存在无主窗口。`CATEGORY_SLUGS` 不得因看似无人使用而删除。

### 需求:GET /rankings 支持按商品名子串搜索（q 参数）

**Reason**: `q` 参数随本变更移除,服务端不再有 `LIKE` 实现。

**Migration**: 搜索改为端上快照内本地筛选,口径由 `api-client` 单方定义(`trim` → 按码点截断 ≤64 → 仅 `A-Z`↔`a-z` 折叠 → 子串)。选 ASCII-only 折叠是为与移除前的命中集合一致——SQLite `LIKE` 仅折叠 ASCII,而 JS `toLowerCase()` 折叠 Unicode。

...
定义无状态 `POST /compute`：对结构化入参做确定性单价计算(tier3),并在所选品类 cohort 内返回位置与邻居；约束其入参/响应、无写库边界和不可计算/跨轴守卫。
...
**重量轴当前拒绝定位**:rankings 快照只含 `per100ml IS NOT NULL` 的容量轴行,因此 `cohortAxis` 解析为 `per100g` 时**必须**返回 `400`、禁止进入定位。只有快照扩出重量轴后才能解禁。
...
- **那么** 必须因当前快照只有 per100ml 容量轴而返回 `400`;只有快照扩出重量轴后才能解禁

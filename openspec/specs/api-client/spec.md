...
- **两条拒绝分支**,缺一则无法安全派生单一 cohort:
  - 目标节点 `comparableUnit` 为 `null` → 拒绝并给出可呈现原因;
  - 目标 slug **不在** `categoryNodes` 中(未播种 / 已下架 / 陈旧快照)→ 返回**可与上一条区分**的拒绝态。
...
**cohort 判定只使用同一快照**:客户端依据该快照 `categoryNodes` 中已解析的 `comparableUnit` 和 `parentSlug` 派生视图。`GET /rankings` 不接收目标节点、没有另一套服务端 cohort 守卫;未播种窗口由「目标 slug 不存在」分支处理。`POST /compute` 虽在服务端使用同一派生 helper,仍是独立的无状态请求。

...
describe("listBoardSnapshot", () => {
  ...
  it("excludes-and-counts a single corrupt row instead of failing the snapshot", async () => {
    ...
    expect(snap.excluded).toEqual([
      { reason: "warnings_undecodable", count: 1 },
    ]);
  });
  ...
  it("raises each remaining exclusion reason under its own name", async () => {
    ...
    t.handle
      .prepare("UPDATE unit_price SET formula = NULL WHERE id = ?")
      .run("up-noformula");
    ...
    t.handle
      .prepare("UPDATE unit_price SET formula = '' WHERE id = ?")
      .run("up-emptyf");
    ...
    expect(
      [...snap.excluded].sort((a, b) => (a.reason < b.reason ? -1 : 1)),
    ).toEqual([
...
describe("listBoardSnapshot projection values", () => {
  ...
  it("takes confidence from unit_price and coalesces the low-price watermark", async () => {
    ...
    t.handle
      .prepare("UPDATE product SET confidence = 0.11 WHERE id = ?")
      .run("prod-v");
    t.handle
      .prepare("UPDATE unit_price SET confidence = 0.99 WHERE id = ?")
      .run("up-v");
    t.handle
      .prepare(
        "UPDATE product_raw SET price = 500, lowest_price = 300 WHERE id = ?",
      )
      .run("raw-v");
    t.handle
      .prepare(
        "UPDATE product_raw SET price = 700, lowest_price = NULL WHERE id = ?",
      )
      .run("raw-w");
    t.handle
      .prepare("UPDATE product_raw SET source_url = ? WHERE id = ?")
      .run("https://x/1", "raw-v");

...
const NODES: SnapshotCategoryNode[] = [
  ...
  { slug: 'milk', name: '牛奶', parentSlug: 'dairy', comparableUnit: 'per_100ml', rankable: true },
  {
    slug: 'yogurt',
    name: '酸奶',
    parentSlug: 'dairy',
...
function row(id: string, per100ml: number, categorySlugs: string[], title = `t-${id}`): SnapshotRow {
  return {
    id,
    title,
    priceCents: 100,
...
describe('parseRankingsSnapshot — object-level invariants', () => {
  ...
  it('rejects duplicate node slugs and a dangling parentSlug', () => {
    expect(() => snap([], [...NODES, NODES[2] as (typeof NODES)[number]])).toThrow(/duplicate/);
    expect(() =>
      snap(
        [],
        [{ slug: 'x', name: 'X', parentSlug: 'nope', comparableUnit: null, rankable: false }],
      ),
    ).toThrow(/has no node/);
  });
  ...
  it('rejects a parent cycle rather than hanging on ancestor resolution', () => {
    expect(() =>
      snap(
        [],
        [
          { slug: 'a', name: 'A', parentSlug: 'b', comparableUnit: null, rankable: false },
          { slug: 'b', name: 'B', parentSlug: 'a', comparableUnit: null, rankable: false },
        ],
      ),
    ).toThrow(/cycle/);
  });
...
describe('cohort derivation', () => {
  ...
  it('surfaces a row attached to a NON-LEAF node under that node and its ancestors', () => {
    ...
    expect(atDairy.ok && atDairy.rows.map((r) => r.id)).toEqual(['nonleaf', 'leaf']);
  ...
  it('preserves the server order through filtering — no client-side sort', () => {
    ...
    const s = snap([row('c', 9, ['milk']), row('a', 1, ['milk']), row('b', 5, ['milk'])]);
...
describe('pageOf', () => {
  const rows = [row('a', 1, ['milk']), row('b', 2, ['milk']), row('c', 3, ['milk'])];
...
describe('parseRankingsSnapshot — jitless', () => {
  ...
  const valid = { rows: [row('a', 1, ['milk'])], categoryNodes: NODES, excluded: [] };

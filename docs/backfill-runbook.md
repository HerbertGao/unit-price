...
**「检测干净」只有在暂停异步 ingest 的窗口内才是持久结论。** `/ingest` 与 `/ingest/batch` 是 `202` + 后台解析,系统**没有 drain 信号**:检测跑出零偏差的那一刻,可能还有在飞的后台单元随后把更旧的观测刷进 `unit_price`,而它**没有后继写者**来收拾。因此:

1. 修正窗口内**暂停 `/ingest` 与 `/ingest/batch` 入口**,并等已受理的后台单元自然结束(无 drain 信号,按经验留足时间);
2. 点名重报**一律走同步 `/contribute`**(`upsertRaw → orchestrate → saveParsed` 全在请求内完成,写完即终态),**禁止**走 `/ingest`——那会重新引入在飞单元;
3. 最终检测在窗口内跑完,**之后**才开放入口。

没有这个边界时,完成判据只能降级为「查询瞬间干净」,并须明示本轮**不给**持久保证。
...
census ② 的偏差谓词是「`formula` 首项(元)按分四舍五入 ≠ `product_raw.price`」,并把结果分成 `drifted_fixable_by_reingest`(可修)与 `drifted_ghost`(幽灵行,重报只会写新键、修不到旧行)。**核对按分相等、禁用浮点容差**——`|首项 − price/100| ≤ 0.01` 会把真正过期一分钱的行判绿。该谓词在 `design.md` D3 另有一份摘录,**两处必须同改**。
...
1. **重抽 HAR 重灌**——命中行的派生值随该次解析落地而刷新。
2. **跑打标签 backfill**(从**空 cursor** 起跑,见「驱动」节):ingest 三条路径都不打标签,重灌中标题漂移产生的新 `product` 行是 `rankable=0`、不入榜,而被弃的旧行仍带 tag 在榜——这一步把新行拉进榜。
3. **跑 `scripts/census-drift.sql`**:取 census ② 的可修偏差行 SKU 列表,与本轮 HAR 的 `(store, storeSku)` **求交**;交集外的行本轮无数据源可重报,直接登记残留。
4. **对交集里的 SKU 经同步 `/contribute` 逐个重报**,回第 3 步。
...
**census ④b(`formula IS NULL AND price > 0`)是人工复核清单、不是失败门**:它无法机械区分「过期的 NULL」(曾 ≤0 价落 NULL、重报正价后解析失败)与「合法不可计算」(正价但无轴 / 规格不一致),区分要按已存 spec 重算,系统不提供该能力。实测 **227 行**,量级说明它以合法不可计算为主;逐轮记数即可,不阻塞收尾。

...
- **v1(随 `add-database` / Phase 3)**:tag 字典(kind + is-a + comparable_unit)、product_tag、store_category_map、category_closure;自动打标签 = tier1 规则 + 山姆 map(LLM 候选稍后)。
  ...
  - **`rankable` 已接入快照榜**:`GET /rankings` 返回全部 `product.rankable=true ∧ per100ml IS NOT NULL` 的行及其 category slugs、同版本品类树,不再按节点查询。客户端沿快照 `parentSlug` 本地派生 cohort,只允许目标节点 `rankable=true` 时呈现榜;root `饮料` / `酒类` 父为 `false`,从而拒绝「水 + 酒」「啤酒 + 威士忌」混排。软饮/乳品/各酒种叶各自为 per100ml cohort。`GET /categories` 与快照节点使用同一无计数形状;需要节点行数时从同一快照派生,不维护 `rankableCount`。
...
| 节点榜入榜判据(P3 前两套并存:扁平榜「仅 `per100ml IS NOT NULL`」vs taxonomy 资格门) | `/rankings` 传输总体 = `rankable=true ∧ per100ml IS NOT NULL`;客户端按同体下发的父链树派生单一 cohort,目标节点 `rankable=false` 时拒绝呈现 | **收敛**:全量快照统一传输总体,软饮为具名落地 cohort;分类、搜索、分页本地派生且保序。`/categories` 与快照节点同形、无 `rankableCount`,榜入口只看 `rankable` |

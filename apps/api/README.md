...
| code | HTTP | 来源 | rawId |
| --- | --- | --- | --- |
| `invalid-request` | 400 | 请求体非法（`title` 空、`price` 非有限、`store`/`storeSku` 空/缺失）；未写任何行、不排程后台 | — |
| `persistence-error` | 500 | DB binding 缺失/不可用，或 `upsertRaw` 写失败；raw 未落、不排程后台 | — |
| `internal` | 500 | 响应体未过校验（`rawId` 恒非空故实质不可达） | — |

`/ingest` 请求路径的错误码集合为 `{ invalid-request(400), persistence-error(500), internal(500), accepted(202) }` 加治理码，**不含** `503 insufficient-information` 与业务 `500 config-error`——因为 `upsertRaw` 成功即 `202`，其后的解析/落库发生在后台、不影响 HTTP 状态。本地 Node dev 不绑定 `DB`，`/ingest` 在取 repository 阶段即走 `persistence-error`、不进后台。

### 后台异步解析

每条 `202` 返回后，于后台触发**一次** tier2 解析 + tier3 单价计算（生产经 `c.executionCtx.waitUntil` 异步执行）。后台按三态分流，失败**只记结构化日志、不影响已返回的 `202`、不重试、不重发 LLM**：

- `ok` → 落 `product` + `unit_price`。**确定不可计算**（`per100ml=null`）照常落库（`unit_price.per100ml` 为 NULL）。
- `insufficient`（标题无规格、tier2 无法补全）→ 只打日志（含 `rawId`/`store`/`storeSku`），保留「有 raw 无 product」中间态供后续重解析，**不**重喂 LLM。
- `config-error`（运行期配置错误）/ `saveParsed` 抛错 → 只打日志，不落 `product`、不重试。

...
describe("toCohorts — derive selectable leaf cohorts from /categories (no hardcode)", () => {
  it("keeps only rankable nodes, preserving server order", () => {
    const cs = toCohorts([
      cat("beverage", {
        rankable: false,
        comparableUnit: null,
      ...
      cat("alcohol", { rankable: false, comparableUnit: null }),
  ...
  it("keeps an empty-but-rankable cohort; the server returns an empty population", () => {
    expect(toCohorts([cat("empty-cohort")])).toEqual([
      { slug: "empty-cohort", name: "empty-cohort", axis: "per_100ml" },
    ]);
  });

  it("skips a per_100g cohort this period (server 400s 重量轴 until backfill; UI must not offer it)", () => {
    const cs = toCohorts([
      cat("snacks", { name: "零食", comparableUnit: "per_100g" }),
    ]);
    expect(cs).toEqual([]);
  });
...
describe("buildComputeRequest — light validation + ComputeRequest assembly (no request on ok:false)", () => {
  it("mode unit: assembles unitSize + quantity, omits totalAmount", () => {
    const r = buildComputeRequest(
      input({ mode: "unit", quantity: "24", amount: "330", unit: "ml" }),
    );
    expect(r).toEqual({
      ok: true,
      request: {
  ...
  it("mode total: assembles totalAmount, omits quantity + unitSize (two-of-one exclusivity)", () => {
    const r = buildComputeRequest(
      input({ mode: "total", amount: "7920", unit: "ml" }),
    );
    expect(r).toEqual({
      ok: true,
      request: {
  ...
  it("no cohort selected → ok:false (请选择品类), no request", () => {
    expect(buildComputeRequest(input({ cohort: undefined }))).toEqual({
      ok: false,
      hint: "请选择品类",
    });
  });
  ...
  it("off-axis unit for the cohort → ok:false with the axis caption (cross-axis pre-guard)", () => {
    const r = buildComputeRequest(
      input({ cohort: cohort({ axis: "per_100ml" }), unit: "g" }),
    );
  ...
  it("non-positive容量 → ok:false, names the right field per mode", () => {
    expect(buildComputeRequest(input({ mode: "unit", amount: "0" }))).toEqual({
      ok: false,
      hint: "请输入单件容量",
    });
    expect(buildComputeRequest(input({ mode: "total", amount: "" }))).toEqual({
      ok: false,
      hint: "请输入总容量",
    });
  });
  ...
  it("mode unit with bad quantity (zero / non-integer / blank) → ok:false", () => {
    expect(buildComputeRequest(input({ mode: "unit", quantity: "0" })).ok).toBe(
      false,
    );
    expect(
      buildComputeRequest(input({ mode: "unit", quantity: "2.5" })).ok,
    ).toBe(false);
    expect(buildComputeRequest(input({ mode: "unit", quantity: "" })).ok).toBe(
      false,
    );
  });
  ...
  it("mode total ignores a blank quantity (quantity not required on the total path)", () => {
    const r = buildComputeRequest(
      input({ mode: "total", quantity: "", amount: "7920" }),
    );
  ...
  it("decimal容量 with L unit assembles as-is (server converts the axis)", () => {
    const r = buildComputeRequest(
      input({ mode: "total", amount: "1.25", unit: "L" }),
    );
    expect(r).toEqual({
      ok: true,
      request: {
...
describe("deriveResultView — result-card presentation (PURE, single source for the card)", () => {
  ...
  it("TIE inputs: pos (dot) never contradicts the verdict (the round-3 F-B regression guard)", () => {
    ...
    const tiedCheap = deriveResultView(
      res({ rank: 1, total: 5, percentile: 60 }),
    );

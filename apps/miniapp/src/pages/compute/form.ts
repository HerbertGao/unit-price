...
/**
 * Derive selectable cohorts from a GET /categories tree. A node is selectable
 * iff it is itself a single comparable cohort (`rankable`). The server no longer
 * publishes a second count that can drift from the snapshot; an empty cohort is
 * still a valid target and `/compute` returns an empty comparison population.
 * Non-rankable group headers (root 饮料 / 酒类 parent) are excluded.
 *
...
export function buildComputeRequest(
 ...
 if (input.mode === "unit") {
  ...
  if (!(Number.isInteger(qty) && qty > 0))
   return { ok: false, hint: "请输入数量（正整数）" };
...
export function deriveResultView(result: ComputeResult): ResultView {
 ...
 const verdict: Verdict =
  cheaperPct >= 66 ? "worth" : cheaperPct <= 34 ? "pricey" : "mid";
